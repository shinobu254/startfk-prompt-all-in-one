...
export default {
    ...
    mounted() {
        ...
        this.gradioAPI.getConfig().then(res => {
            ...
            this.$toastr.error("Connection to backend API service failed. Please manually refresh the webpage. If it still doesn't work, please restart the Panel.<br/>连接到后台接口服务失败，请手动再刷新一次网页。如果还是不行，请重启Panel。<br/><br/>" + err.message, globals.shortName, {timeOut: 20000})
            console.log(err)
        })
    },
    methods: {
                ...
                if (data.tagCompleteFile !== null) {
                    ...
                    /*if (typeof TAC_CFG === 'object' && typeof QUEUE_FILE_LOAD === 'object') {
                        QUEUE_FILE_LOAD.push(() => {
                            ...
                            this.tagCompleteFile = '\\extensions\\startfk-tagcomplete\\tags\\' + TAC_CFG.translation.translationFile
                            this.$refs.translateSetting.getCSV(this.tagCompleteFile)
                        })
                    }*/
                }
