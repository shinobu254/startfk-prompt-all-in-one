...
try:
    ...
    if cmd_opts.data_dir:
        ...
        startfk_dir = os.path.normpath(Path().absolute()) + os.path.sep
        if not extension_dir.startswith(startfk_dir):
            find = False
            ...
            if not find:
                message = f'''
\033[1;31m[startfk-prompt-all-in-one]
...
try:
    ...
    print('startfk-prompt-all-in-one background API service started successfully.')
except Exception as e:
    print(f'startfk-prompt-all-in-one background API service failed to start: {e}')
